...
#                 return JsonResponse({"error": "Missing required columns."})
...
#             df['IsolationForest'] = ["Fraud" if i == -1 else "Normal" for i in iso_pred]
#             df['DBSCAN'] = ["Fraud" if i == -1 else "Normal" for i in dbscan_pred]

#             print("✅ Prediction done")
#             return JsonResponse({"predictions": df.to_dict(orient="records")})
...
# DATA_STORE_PATH = os.path.join("Apps", "homeApp", "data_store.csv")
...
#             print(f"✅ CSV loaded {new_data.shape}")
...
#                 return JsonResponse({"error": "Missing required features in uploaded data"})

#             # Load previous data
#             if os.path.exists(DATA_STORE_PATH):
#                 try:
#                     stored_data = pd.read_csv(DATA_STORE_PATH)
#                     print(f"📦 Loaded stored data: {stored_data.shape}")
#                 except Exception as e:
#                     print(f"❌ Failed to read stored data: {e}")
#                     stored_data = pd.DataFrame(columns=required_columns)
#             else:
#                 stored_data = pd.DataFrame(columns=required_columns)
#                 print("📄 No previous data, creating new storage")

#             # Combine and save
#             combined_data = pd.concat([stored_data, new_data], ignore_index=True)
#             combined_data.to_csv(DATA_STORE_PATH, index=False)
#             print(f"📁 Combined and saved data: {combined_data.shape}")

#             # Reload to ensure it’s written properly
#             reloaded_data = pd.read_csv(DATA_STORE_PATH)
#             print(f"📂 Reloaded data: {reloaded_data.shape}")

#             # Preprocess
#             scaler = StandardScaler()
#             X_scaled = scaler.fit_transform(reloaded_data[required_columns])
...
#             # Predict on new uploaded chunk
#             new_scaled = scaler.transform(new_data[required_columns])
#             iso_pred = iso.predict(new_scaled)
#             dbscan_pred = dbscan.fit_predict(new_scaled)

#             new_data["IsolationForest_Prediction"] = ["Fraud" if x == -1 else "Normal" for x in iso_pred]
#             new_data["DBSCAN_Prediction"] = ["Fraud" if x == -1 else "Normal" for x in dbscan_pred]

#             return JsonResponse({"predictions": new_data.to_dict(orient="records")})
...
#             known_frauds = combined_data[combined_data['Actual'] == 'Fraud']
#             fraud_upsampled = pd.concat([known_frauds] * 10, ignore_index=True)

#             non_features = ['Class', 'Prediction', 'Actual', 'Correct']
#             feature_cols = [col for col in new_data.columns if col not in non_features]
#             X_train = pd.concat([
#                 combined_data[feature_cols],
#                 fraud_upsampled[feature_cols]
#             ], ignore_index=True)

#             X_train = X_train.select_dtypes(include=['number']).dropna()

#             model = IsolationForest(n_estimators=100, contamination=0.001, random_state=42)
#             model.fit(X_train)

#             X_new = new_data[feature_cols].select_dtypes(include=['number']).dropna()
#             preds = model.predict(X_new)
#             new_data['Prediction'] = ['Fraud' if p == -1 else 'Normal' for p in preds]
...
#             return JsonResponse({
#                 "predictions": new_data[['Prediction', 'Actual', 'Correct']].to_dict(orient='records'),
...
#         print(f"✅ Uploaded CSV loaded successfully: {uploaded_data.shape}")
#         print(f"📋 Columns in the uploaded file: {uploaded_data.columns.tolist()}")

#         # 2. Load previous saved data
#         past_data = pd.read_csv(DATA_PATH, skipinitialspace=True)
#         print(f"📦 Loaded previous data: {past_data.shape}")

#         # 🔥 PATCH: Align columns if missing
#         missing_cols = set(uploaded_data.columns) - set(past_data.columns)
#         for col in missing_cols:
#             past_data[col] = 0  # or np.nan
#         extra_cols = set(past_data.columns) - set(uploaded_data.columns)
#         for col in extra_cols:
#             uploaded_data[col] = 0  # or np.nan

#         # 3. Combine past data and newly uploaded data
#         combined_data = pd.concat([past_data, uploaded_data], ignore_index=True)
#         print(f"🔗 Combined dataset size: {combined_data.shape}")

#         # 4. Optional: upsample if needed (your existing code)

#         # 5. Separate features
#         feature_cols = [col for col in uploaded_data.columns if col not in ['Class']]
#         print(f"🛠 Training on features: {feature_cols}")

#         X_train = combined_data[feature_cols]
#         X_predict = uploaded_data[feature_cols]
...
#         preds = np.where(preds == -1, "Fraud", "Normal")

#         # 8. If Class present, evaluate
#         if 'Class' in uploaded_data.columns:
#             true_labels = uploaded_data['Class'].map({0: 'Normal', 1: 'Fraud'})
...
#             print(results)
#         else:
#             print("No actual Class labels provided for evaluation.")

#         # 9. Done
#         return JsonResponse({'status': 'success', 'predictions': preds.tolist()})
...
#     - Uses Isolation Forest to detect anomalies (fraud)
...
#         # Upsample fraud cases to improve detection
#         known_frauds = combined_data[combined_data['Actual'] == 'Fraud']
#         if not known_frauds.empty:
#             # Scale upsampling based on the dataset size
#             upsample_factor = min(10, max(2, int(100000 / (len(known_frauds) + 1))))
#             fraud_upsampled = pd.concat([known_frauds] * upsample_factor, ignore_index=True)
#         else:
#             fraud_upsampled = pd.DataFrame(columns=combined_data.columns)
...
#         X_train_base = combined_data[numeric_cols]
#         X_train_fraud = fraud_upsampled[numeric_cols] if not fraud_upsampled.empty else pd.DataFrame(columns=numeric_cols)
        
#         # Prepare training data
#         X_train = pd.concat([X_train_base, X_train_fraud], ignore_index=True)
#         X_train = X_train.fillna(X_train.mean())
        
#         if X_train.empty or X_train.shape[1] == 0:
#             return JsonResponse({"error": "No numeric features available for model training"})
        
#         # Train Isolation Forest model with optimized parameters
#         # Adjust contamination based on data characteristics
#         contamination = 0.001  # Default low contamination
...
#                 contamination = max(0.001, min(0.1, known_fraud_rate * 1.5))
...
#         # Process new data for prediction - use only columns that were in the training data
#         # This ensures we only use features that the model was trained on
#         try:
#             X_new = new_data[numeric_cols].copy()
#             # Fill missing values with the mean from training data
#             for col in numeric_cols:
#                 if col in X_new.columns:
#                     X_new[col] = X_new[col].fillna(X_train[col].mean() if not X_train[col].empty else 0)
...
#         # Make predictions - handle possible errors
#         try:
#             preds = model.predict(X_new)
#             new_data['Prediction'] = ['Fraud' if p == -1 else 'Normal' for p in preds]
#         except ValueError as e:
#             # Handle the specific "Feature names unseen at fit time" error
#             if "Feature names unseen at fit time" in str(e):
#                 # Extract the problematic column names from the error message
#                 import re
#                 unseen_features = re.search(r'Feature names unseen at fit time: \[(.*?)\]', str(e))
#                 unseen_features = unseen_features.group(1) if unseen_features else "unknown features"
                
#                 return JsonResponse({
#                     "error": f"Model training and prediction data mismatch. Unseen features: {unseen_features}. Please ensure consistent column names across all data."
#                 }, status=400)
#             else:
#                 raise
...
#         return JsonResponse({
#             "predictions": new_data[['Prediction', 'Actual', 'Correct']].fillna("Unknown").to_dict(orient='records'),
